...
Overit, ze Airflow lze monitorovat pres Zabbix (HTTP Agent → REST API). Cil: sledovat health komponent (scheduler, dag-processor, triggerer), automaticky alertovat pri selhani. Alternativa k poc04 pro prostredi kde uz Zabbix bezi. Detaily viz [ANA-09](../analyses/ANA-09_monitoring_zabbix.md).

Vychazi z [poc04](../poc04_monitoring/README.md) (Prometheus + Grafana).
...
Na rozdil od poc04 (StatsD → Prometheus), Zabbix **primo vola Airflow REST API**:
...
## Srovnani s poc04 (Prometheus + Grafana)

| | poc04 (Prometheus) | poc05 (Zabbix) |
|---|---|---|
